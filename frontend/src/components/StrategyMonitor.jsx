export default function StrategyMonitor({ strategyData }) {
    ...
    return (
        <div className="bg-vault-card border border-vault-border rounded-xl p-6">
            ...
            <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold mb-4 ${isHighVol ? "bg-vault-red/20 text-vault-red border border-vault-red/30" : "bg-vault-green/20 text-vault-green border border-vault-green/30"}`}>
            ...
            <div className="space-y-3">
                {[
                    ...
                    ["Strike Price", `$${strategyData.strikePrice}`, "text-vault-yellow"],
                    ["Collateral Ratio", `${strategyData.collateralRatio}%`, "text-white"],
                    ["Multiplier", isHighVol ? "+20% (High Vol)" : "+10% (Low Vol)", "text-gray-300"]
                ].map(([k, v, c]) => (
